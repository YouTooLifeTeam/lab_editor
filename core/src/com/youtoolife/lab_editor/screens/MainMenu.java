...
public class MainMenu extends ScreenAdapter {
	...
	TextButton swipeBtn, saveBtn, loadBtn, copyBtn, editSize, clearAll,
	...
	public void createGui() {
		...
		textArea = new TextArea(
				"Maze Editor ver. alpha by YouTooLife Team (c)\n<------------------>\n\n",
				skin);
		...
		if (file.isDirectory()) {
			...
			for (int i = 0; i < files.length; i++) {
				if (files[i].name().contains(".png")
					...
					sprite.setPosition(1024 - 50 * 3 - 5 * 3 + 55 * posX,
							764 - 55 * posY - 150);
					images.add(sprite);
	...
	public void loadChunk() {
		...
		textArea.setText(textArea.getText() + "\n" + "File '"
				+ fileField.getText() + "' has been loaded!");
	}
	...
	public void createChunk() {
		...
		RootElement.setAttribute(
				"name",
				fileField.getText().substring(0,
						fileField.getText().indexOf(".")));
		...
		for (int i = 0; i < holsts.size; i++) {
			...
			if (holsts.get(i).type == "")
				NameElementTitle.setAttribute("type", "floor");
			NameElementTitle.setAttribute("img", holsts.get(i).img);
			NameElementTitle.setAttribute("x",
		...
		try {
			...
			t.transform(new DOMSource(doc), new StreamResult(
					new FileOutputStream(fileField.getText())));
		...
		textArea.setText(textArea.getText() + "\n" + "File '"
				+ fileField.getText() + "' has been created!");
		/*
		 * textArea.clear();
		 * textArea.setText("boolean["+ySize+"]["+xSize+"] = {\n"); for (int y =
		 * 0; y < ySize; y++) { textArea.setText(textArea.getText()+"{"); for
		 * (int i = 0; i < xSize; i++) {
		 * textArea.setText(textArea.getText()+(arr[y][i]?"1":"0")); if (i !=
		 * (xSize-1)) textArea.setText(textArea.getText()+","); }
		 * textArea.setText(textArea.getText()+"}"); if (y != (ySize-1))
		 * textArea.setText(textArea.getText()+",\n"); }
		 * textArea.setText(textArea.getText()+"\n}");
		 */
	}
	...
	public void createHolsts() {
		if (xSize < Integer.parseInt(xField.getText())) {
			for (int i = (xSize > 0 ? (xSize - 1) : 0); i < Integer
					.parseInt(xField.getText()); i++) {
				for (int y = 0; y < Integer.parseInt(yField.getText()); y++) {
					Box led = new Box(Assets.rectRegion, new Vector2(
							212 + i * 50, 609 - y * 50),
		...
		if (ySize < Integer.parseInt(yField.getText())) {
			for (int i = 0; i < xSize; i++) {
				for (int y = (ySize > 0 ? (ySize - 1) : 0); y < Integer
	...
	public void inputHandler(float delta) {
		...
		if (Gdx.input.justTouched()) {
			...
			float w = Gdx.graphics.getWidth(), h = Gdx.graphics.getHeight(), dw = w / 1024, dh = h / 764, cX = Gdx.input
					.getX(), cY = Gdx.input.getY(), x = cX / dw, y = 764 - cY
					/ dh;
